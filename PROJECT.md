...
3. **Customer Home** (`CustomerHome.jsx`) — a real dashboard now, not just a greeting: Active/Completed request stat cards, "Describe your problem" CTA, a real "Recent requests" list, an **AI-powered provider search box** ("Ask AI to find a provider" — calls `POST /classify` to read free text like "find the best rated plumer", then maps the answer onto a real catalog row via `lib/matching.js` — alias, substring and edit-distance matching, so typos and synonyms both work, and the search still resolves locally when the ML service is asleep), and "Browse by category" chips for every service (replacing the old single fake "Recommended" card); bottom nav (Home / Requests / Profile)
