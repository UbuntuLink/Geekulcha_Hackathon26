...
3. **Customer Home** (`CustomerHome.jsx`) — a real dashboard now, not just a greeting: Active/Completed request stat cards, "Describe your problem" CTA, a real "Recent requests" list, an **AI-powered provider search box** ("Ask AI to find a provider" — calls `POST /classify` to extract a category from free text like "cheap plumber available today", then applies keyword heuristics against the real matching endpoint), and "Browse by category" chips for every service (replacing the old single fake "Recommended" card); bottom nav (Home / Requests / Profile)
