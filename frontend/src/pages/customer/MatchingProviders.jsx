...
export default function MatchingProviders() {
  ...
  return (
    <Screen
      title={providers ? `${providers.length} provider${providers.length === 1 ? "" : "s"} found` : "Finding providers..."}
      subtitle={providers?.length ? "Explore the matches, change the sort, and open any provider to see more." : "Searching for providers who match your request."}
      eyebrow="Step 3 of 3"
      size="wide"
    >
      ...
      {providers === null && !error && (
        <div className="rounded-2xl border border-brand/15 bg-brand-mist px-4">
          <Loading label="Checking local providers..." />
        </div>
      )}
      {providers?.length === 0 && <EmptyState>No providers offer this service yet — check back soon.</EmptyState>}
